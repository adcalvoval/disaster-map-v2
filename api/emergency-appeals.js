...
module.exports = async (req, res) => {
    ...
    try {
        ...
        console.log(`Fetching Emergency Appeals from IFRC API...`);
        ...
        // Fetch emergency appeals with atype=1 (Emergency Appeal) and status=0 (Active)
        const url = `${IFRC_GO_API_BASE_URL}/appeal/?atype=1&status=0&limit=${limit}&offset=${offset}`;
        console.log(`Fetching Emergency Appeals from: ${url}`);

        const response = await axios.get(url, { headers, timeout: 30000 });
        const apiData = response.data;

        if (apiData && apiData.results) {
            console.log(`Retrieved ${apiData.results.length} Emergency Appeals from IFRC API`);
            console.log(`Total available in API: ${apiData.count || 'unknown'}`);

            // Debug: Log the structure of the first appeal
            if (apiData.results.length > 0) {
                console.log('First Appeal structure:', JSON.stringify(apiData.results[0], null, 2));
            }
            ...
            apiData.results.forEach(appeal => {
                ...
                if (!coords) {
                    console.log(`Filtered out Appeal ${appeal.id}: no coordinates for country ${countryISO3}`);
                    return;
                }

                allAppeals.push({
                    id: appeal.id,
                    name: appeal.name || 'Unknown Appeal',
                    country: appeal.country?.name || 'Unknown',
                    country_iso3: countryISO3,
                    disaster_type: appeal.dtype?.name || 'Unknown',
                    start_date: appeal.start_date,
                    end_date: appeal.end_date,
                    amount_requested: appeal.amount_requested || 0,
                    amount_funded: appeal.amount_funded || 0,
                    status: appeal.status_display || 'Active',
                    latitude: coords.lat,
                    longitude: coords.lng,
                    api_source: true
                });

                console.log(`✅ Added Appeal ${appeal.id}: ${appeal.name} in ${appeal.country?.name}`);
            });

            console.log(`Returning ${allAppeals.length} active Emergency Appeals (from ${apiData.results.length} total)`);

            return res.status(200).json({
                success: true,
                count: allAppeals.length,
                total: apiData.count || allAppeals.length,
                appeals: allAppeals,
                source: 'IFRC API',
                pagination: {
        ...
        console.error('Error loading Emergency Appeals:', error.message);
        ...
        const sampleAppeals = getSampleAppeals();

        res.status(200).json({
            success: true,
            count: sampleAppeals.length,
            total: sampleAppeals.length,
            appeals: sampleAppeals,
...
// Fallback sample appeal data
function getSampleAppeals() {
    return [
        {
            id: 'sample_appeal_1',
            name: 'Bangladesh: Floods 2024',
            country: 'Bangladesh',
            country_iso3: 'BGD',
            disaster_type: 'Flood',
        ...
        },
        {
            ...
            end_date: null,
            amount_requested: 7000000,
            amount_funded: 4200000,
            status: 'Active',
            latitude: 14.5995,
            longitude: 120.9842,
            api_source: false
