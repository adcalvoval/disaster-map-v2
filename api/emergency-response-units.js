...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (apiData && apiData.results) {
            ...
            // Debug: Log the structure of the first ERU
            if (apiData.results.length > 0) {
                console.log('First ERU structure:', JSON.stringify(apiData.results[0], null, 2));
            }

            // Process the events and their active ERUs
            let allERUs = [];

            apiData.results.forEach(event => {
                if (event.active_erus && event.active_erus.length > 0) {
                    console.log(`Processing event "${event.name}" with ${event.active_erus.length} active ERUs`);

                    event.active_erus.forEach(eru => {
                        // Check if ERU is still active (no end_date or end_date in future)
                        const now = new Date();
                        const endDate = eru.end_date ? new Date(eru.end_date) : null;
                        const isActive = !endDate || endDate > now;

                        if (!isActive) {
                            console.log(`Filtered out ERU ${eru.id}: ended on ${eru.end_date}`);
                            return;
                        }

                        // Get country coordinates (we'll need to add these manually or use a service)
                        const coords = getCountryCoordinates(eru.deployed_to.iso3);

                        if (!coords) {
                            console.log(`Filtered out ERU ${eru.id}: no coordinates for country ${eru.deployed_to.iso3}`);
                            return;
                        }
                        ...
                        allERUs.push({
                            ...
                            deploying_society: eru.eru_owner_details?.national_society_country_details?.society_name || 'Unknown',
                            ...
                            event_id: event.id,
                            units: eru.units || 0,
                            equipment_units: eru.equipment_units || 0,
                            api_source: true
                        });

                        console.log(`✅ Added ERU ${eru.id}: ${eru.type_display} in ${eru.deployed_to.name}`);
                    });
                } else {
                    console.log(`Event "${event.name}" has no active ERUs`);
                }
            });

            console.log(`Returning ${allERUs.length} active ERUs (from ${apiData.results.length} events)`);

            const transformedERUs = allERUs;
